...
from model.position_layer import PositionalEncoding
import torch.nn.functional as F


class CompGCN(nn.Module):
    def __init__(self, num_ent, num_rel, num_base, num_time, init_dim, gcn_dim, embed_dim, n_layer, edge_type, edge_norm,
                 conv_bias=True, gcn_drop=0., opn='mult'):
        super(CompGCN, self).__init__()
        self.act = torch.tanh  # 是否可以修改
        self.loss = nn.BCELoss()  # 其他损失函数？
        self.num_ent, self.num_rel, self.num_base = num_ent, num_rel, num_base
        self.num_time_diff = num_time
        ...
        self.bias = nn.Parameter(torch.zeros(self.num_ent))  # E ? 每个entity作为候选的bias

        # 时间差的编码
        self.time_diff_embed = self.get_param([self.num_ent, self.init_dim])
        self.time_embeder = PositionalEncoding(self.init_dim)
    ...
    def forward_base(self, g, subj, rel, query_time, mask_index, drop1, drop2):  # 增加时间 query_time只是一个数
        """
        :param g: graph
        ...
        e_time = g.edata['time']
        time_diff = query_time - e_time
        time_diff_emd = torch.index_select(self.time_diff_embed, 0, time_diff)
        e_time_emd = self.time_embeder(e_time)
        query_time_emd = self.time_embeder(query_time).expand_as(e_time_emd)
        # print(e_time_emd.size())
        # print(query_time_emd.size())

        g.edata['time_diff'] = time_diff_emd
        g.edata['time_emd'] = torch.cat([e_time_emd, query_time_emd], dim=1)
        if mask_index != None:
            g.edata['mask_index'] = mask_index

        x, r = self.init_embed, self.init_rel  # embedding of relations
        # x, r = self.conv1(g, x, r, self.edge_type, self.edge_norm)
        x, r = self.conv1(g, x, r, mask_index)
        ...
        x, r = self.conv2(g, x, r, mask_index) if self.n_layer == 2 else (x, r)
...
class CompGCN_DistMult(CompGCN):
    def __init__(self, num_ent, num_rel, num_base, num_time, init_dim, gcn_dim, embed_dim, n_layer, edge_type, edge_norm,
        ...
        super(CompGCN_DistMult, self).__init__(num_ent, num_rel, num_base, num_time, init_dim, gcn_dim, embed_dim, n_layer,
                                               edge_type, edge_norm, bias, gcn_drop, opn)
        self.drop = nn.Dropout(hid_drop)

    def forward(self, g, subj, rel, query_time, mask_index=None):
        """
        :param g: dgl graph
        ...
        """
        sub_emb, rel_emb, all_ent = self.forward_base(g, subj, rel, query_time, mask_index, self.drop, self.drop)
