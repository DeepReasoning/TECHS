...
from utils.data_process import build_graph
...
def load_yago_wiki_triples(data_file, rel_num):
    facts = []
    with open(data_file, encoding='utf8') as f:
        for line in f:
            tmp = line.strip().split()[:4]
            facts.append([int(item) for item in tmp])
    facts = np.array(facts)
    h, r, t, ts = facts[:,0], facts[:,1], facts[:,2], facts[:,3]
    reverse_facts = np.stack([t, r+rel_num, h, ts], axis=1)
    facts = np.concatenate([facts, reverse_facts], axis=0)
    # print(facts.shape, facts[:10], facts[-10:])
    return facts
...
class tKGDataset(Dataset):  # dataset  # 后向、前向都尝试
    def __init__(self, data_dir, data_type='train', logic=False, logic_ratio=0.5):  # train valid test
        ...
        if 'YAGO' in data_dir:
            self.n_ent = 10623
            self.n_rel = 10  # 原始rel数量、未添加逆关系
            self.n_ts = 189  # timestemp数量
            self.train_facts = load_yago_wiki_triples(join(self.data_dir, 'train.txt'), self.n_rel)
            self.valid_facts = load_yago_wiki_triples(join(self.data_dir, 'valid.txt'), self.n_rel)
            self.test_facts = load_yago_wiki_triples(join(self.data_dir, 'test.txt'), self.n_rel)

        elif 'WIKI' in data_dir:
            self.n_ent = 12554
            self.n_rel = 24  # 原始rel数量、未添加逆关系
            self.n_ts = 232  # timestemp数量
            self.train_facts = load_yago_wiki_triples(join(self.data_dir, 'train.txt'), self.n_rel)
            self.valid_facts = load_yago_wiki_triples(join(self.data_dir, 'valid.txt'), self.n_rel)
            self.test_facts = load_yago_wiki_triples(join(self.data_dir, 'test.txt'), self.n_rel)
    ...
    def dataset_describe(self):
        ...
        print('Train Num:{} Real Train Num:{} Valid Num:{} Test Num:{}'.format(len(self.train_facts), len(self.train_facts_new),
                                                                               len(self.valid_facts), len(self.test_facts)))
            ...
            if len(neighbor_index) > sample_nodes:
                if mask_2:
                    if sample_method == 1:
                        ...
                        neighbor_ts = neighbor_ts[mask_] - tmp_time  # 负数
...
#     h, r, t, ts, y = map(list, zip(*samples))
#     h = torch.LongTensor(np.array(h))
#     r = torch.LongTensor(np.array(r))
#     t = torch.LongTensor(np.array(t))
#     ts = torch.LongTensor(np.array(ts))
...
if __name__ == '__main__':
    ...
    _ent = np.array([1,2,100,1000])
    query_time = np.array([300,200,300,300])

    # a, b = tkg_dataset.load_neighbors_by_array(_ent, query_time, max_nodes=15)
    # print(a,b)
    # print(tkg_dataset.all_facts[b])
    # index_ = tkg_dataset.all_facts[:,0] == 1
    # print(tkg_dataset.all_facts[index_])
